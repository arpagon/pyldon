...
allowed-tools: Read, Write, Glob, Grep, Task, Bash(cat:*), Bash(ls:*), Bash(tree:*), Bash(uv:*)
---
...
1. **GEMINI_API_KEY**: Must be set as environment variable (passed via container env)
2. **uv**: Available in PATH (pre-installed in container)
3. **Executable**: `.agents/skills/nanobanana-skill/nanobanana.py`
...
2. Run the nanobanana script with appropriate parameters:
   ...
   ```bash
   uv run --with google-genai --with Pillow --with python-dotenv .agents/skills/nanobanana-skill/nanobanana.py --prompt "description of image" --output "filename.png"
   ```
...
2. Run with input images:
   ...
   ```bash
   uv run --with google-genai --with Pillow --with python-dotenv .agents/skills/nanobanana-skill/nanobanana.py --prompt "editing instructions" --input image1.png image2.png --output "edited.png"
   ```
...
- `gemini-3.1-flash-image-preview` (default) - Fast and high quality
...
```bash
uv run --with google-genai --with Pillow --with python-dotenv .agents/skills/nanobanana-skill/nanobanana.py --prompt "A serene mountain landscape at sunset with a lake"
```

### Generate with specific size and output

```bash
uv run --with google-genai --with Pillow --with python-dotenv .agents/skills/nanobanana-skill/nanobanana.py \
...
- Check that `GEMINI_API_KEY` is set in the environment
